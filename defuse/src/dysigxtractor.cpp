...
defuse::DYSIGXtractor::DYSIGXtractor(
	DYSIGXtractor::FrameSelection _frameSelection,
	int _maxFrames, 
	bool _resetTracking,
	int _initSeeds,
	int _initialCentroids, int _iterations, 
	int _minClusterSize, float _minDistance,
	float _dropThreshold, SamplePoints::Distribution _distribution, 
	SamplePoints* _samplepoints,
	int _grayscaleBits, int _windowRadius)
{
	mFrameSelection = _frameSelection;
	...
	if(_samplepoints == nullptr)
	{
		mSamplepoints = new SamplePoints(mInitSeeds, mDistribution);
	}
}

defuse::DYSIGXtractor::DYSIGXtractor(
	int _maxFrames, int _initSeeds, 
	int _initialCentroids, std::string _samplepointdir, SamplePoints::Distribution _distribution)
	: DYSIGXtractor()
{
	mMaxFrames = _maxFrames;
	mInitSeeds = _initSeeds;
	...
	if(_samplepointdir != "")
	{
		Directory samplepointdir(_samplepointdir);
		mSamplepoints = new SamplePoints(_initSeeds, mDistribution);
		File sampleFile(samplepointdir.getPath(), mSamplepoints->getSamplePointFileName());

		bool status;
		if (!cplusutil::FileIO::isValidPathToFile(sampleFile.getFile()))
		{
			status = mSamplepoints->serialize(sampleFile.getFile());
			LOG_INFO("Samplepoint File: " << sampleFile.getFilename() << " does not exists. A new one will be created.");
		}
		else
		{
			status = mSamplepoints->deserialize(sampleFile.getFile());
			int desirelizedSize = mSamplepoints->getPoints().size();
			if (_initSeeds != desirelizedSize)
			{
				LOG_FATAL("Fatal error: Sample count: " << _initSeeds << " do not match with desirelized samplepoints: " << desirelizedSize);
			}
		}
	}
}

defuse::Features* defuse::DYSIGXtractor::xtract(VideoBase* _videobase)
{
	...
	computeSignatures(stream, vectors);
	size_t e1_end = double(cv::getTickCount());
	double elapsed_secs = (e1_end - e1_start) / double(cv::getTickFrequency());

	Features* features = new Features(_videobase->mFile->getFilename(), vectors);
...
void defuse::DYSIGXtractor::getMotionDirection(std::vector<uchar>& statusVector, std::vector<float>& errorVector, std::vector<cv::Point2f>& prevPoints, std::vector<cv::Point2f>& currPoints, int height, int width, cv::Mat& out) const
{
	cv::Mat samples;
	out.copyTo(samples);
...
void defuse::DYSIGXtractor::getTemporalSamples(cv::Mat& tsamples, cv::Mat& signatures) const
{
	...
	tsamples(cv::Rect(0, 0, tsamples.cols, mMaxClusters)).copyTo(clusters);
	...
	for (std::size_t iteration = 0; iteration < mIterations; iteration++)
	{
		...
		for (std::size_t i = 0; (int)i < tmpCentroids.rows; ++i)
		{
			...
			if (clusters.at<float>(i, as_integer(IDX::WEIGHT)) >(iteration + 1) * mMinimalClusterSize)
			{
...
void defuse::DYSIGXtractor::computeSignatures(cv::VideoCapture& _video, cv::OutputArray _signatures) const
{
	cv::Mat signatures;
...
void defuse::DYSIGXtractor::joinCloseClusters(cv::Mat clusters) const
{
	for (int i = 0; i < clusters.rows - 1; i++)
	{
		...
		{
			if (clusters.at<float>(j, as_integer(IDX::WEIGHT)) > 0 && computeL2(clusters, i, clusters, j) <= mMinimalDistance)
			{
...
void defuse::DYSIGXtractor::dropLightPoints(cv::Mat& clusters) const
{
	int frontIdx = 0;

	// Skip leading continuous part of weighted-enough points.
	while (frontIdx < clusters.rows && clusters.at<float>(frontIdx, as_integer(IDX::WEIGHT)) > mClusterDropThreshold)
	{
		++frontIdx;
	}
	...
	{
		if (clusters.at<float>(frontIdx, as_integer(IDX::WEIGHT)) > mClusterDropThreshold)
		{
...
int defuse::DYSIGXtractor::findClosestCluster(cv::Mat& clusters, cv::Mat& points, int pointIdx) const
{
	int iClosest = 0;
...
void defuse::DYSIGXtractor::normalizeWeights(cv::Mat& clusters) const
{
	// get max weight
...
// ReSharper disable once CppMemberFunctionMayBeStatic
void defuse::DYSIGXtractor::singleClusterFallback(const cv::Mat& points, cv::Mat& clusters) const
{
	if (clusters.rows != 0)
	{
		return;
...
void defuse::DYSIGXtractor::cropClusters(cv::Mat& clusters) const
{
...
float defuse::DYSIGXtractor::computeL2(cv::Mat& _f1, int idx1, cv::Mat& _f2, int idx2) const
{
	double result = 0.0;
...
void defuse::DYSIGXtractor::draw2DSampleSignature(const cv::InputArray _source, const cv::InputArray _signature, cv::OutputArray& _result) const
{
...
double defuse::DYSIGXtractor::square(int a) const
{
	return a * a;
}

void defuse::DYSIGXtractor::drawLine(cv::Mat& image, cv::Point p, cv::Point q, int scale) const
{
...
void defuse::DYSIGXtractor::draw2DFSSignature(cv::InputArray _source, cv::InputArray _signature, cv::OutputArray _result) const
{
...
void defuse::DYSIGXtractor::drawSignatureMotionDirection(cv::InputArray _tempSig, cv::OutputArray _result, int _width, int _height) const
{
	//white background
